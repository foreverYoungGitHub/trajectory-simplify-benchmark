...
def visualize_sequences(tracks, img_folder, output_folder, padding_num=6, create_video=True):
  ...
  for t in tqdm.trange(1, max_frames_seq + 1):
    ...
    if os.path.exists(filename_t + ".png"):
      ...
      logger.warning("Image file not found for " + filename_t + ".png/.jpg, continuing...")
      continue
    ...
    for obj in framedata:
      ...
      cv2.rectangle(img, obj[2:4], obj[2:4]+obj[4:6], color, thickness=1, lineType=cv2.LINE_AA)

    cv2.imwrite(str(output_folder / f"{t:0{padding_num}d}.jpg"), img)

  if create_video:
    os.chdir(output_folder)
    call(["ffmpeg", "-framerate", "10", "-y", "-i", f"%0{padding_num}d.jpg", "-c:v", "libx264", "-profile:v", "high", "-crf", "20",
...
def main(cfg: omegaconf.dictconfig.DictConfig) -> None:
  ...
  for key in cfg.seq_name:
      tracks_file = hydra.utils.to_absolute_path(cfg.tracks_pattern.format(seq_name=key))
      tracks = np.loadtxt(tracks_file, delimiter=",").astype(int)
      if tracks.shape[1] > 6:
        # tracks = tracks[tracks[:,6] == 1]
        tracks = tracks[(tracks[:,7] <= 7)]
      # print(np.unique(tracks[:,7]))
      # continue
      img_folder = Path(hydra.utils.to_absolute_path(cfg.img_pattern.format(seq_name=key)))
      output_folder = Path(hydra.utils.to_absolute_path(cfg.output_pattern.format(seq_name=key)))
